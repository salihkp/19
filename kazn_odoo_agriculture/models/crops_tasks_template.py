...
class CropsTasksTemplate(models.Model):
    ...
    animal_ids = fields.One2many('crops.animals','crops_tasks_template_id',string="Animals",required=True)
    fleet_ids = fields.One2many('crops.fleet','crops_tasks_template_id',string="Fleets",required=True)
    equipment_ids = fields.Many2many('maintenance.equipment',string='Equipments',required=True)